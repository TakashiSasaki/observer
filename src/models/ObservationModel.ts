...
export class ObservationSetModel implements ObservationSet {
  ...
  constructor(initData: Partial<ObservationSet> & { type: ObservationType; title: string }) {
    ...
    this.observations = initData.observations || (initData as any).subObservations || [];
    this.observationIds = initData.observationIds || this.observations.map((o) => o.id);
  ...
  public static fromFirestore(id: string, data: Record<string, any>): ObservationSetModel {
    ...
    const obsArray = Array.isArray(data.observations)
      ? data.observations
      : Array.isArray(data.subObservations)
      ? data.subObservations
      : [];

    const obsIds = Array.isArray(data.observationIds)
      ? data.observationIds
      : obsArray.map((o: any) => o.id);

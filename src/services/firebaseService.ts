...
export async function createObservation(obsData: Omit<ObservationSet, 'id'>): Promise<ObservationSet> {
  ...
  try {
    if (auth.currentUser) {
      // 1. Save individual sub-observations to /singleObservations collection if present
      if (model.observations && model.observations.length > 0) {
        for (const singleObs of model.observations) {
          try {
            await addDoc(collection(db, 'singleObservations'), {
              ...singleObs,
            ...
            console.warn('Single observation write error:', e);
          }
        }
      }

      // 2. Save observation set to /observations collection
...
export async function updateObservationVisibility(
  ...
  try {
    if (auth.currentUser && !id.startsWith('obs_')) {
      const docRef = doc(db, COLLECTION_NAME, id);
      await updateDoc(docRef, { visibility: newVisibility, allowedEmails });
    }
  } catch (e) {
  ...
  // Sync local cache
  const local = getLocalObservations();
  const updated = local.map((item) => (item.id === id ? { ...item, visibility: newVisibility, allowedEmails } : item));
